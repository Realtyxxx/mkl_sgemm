...
int main(void) {
  ...
  printf("\nDGEMM uses %d bytes in %d buffers", AllocatedBytes,
         N_AllocatedBuffers);
  ...
  printf(
      "\nPeak memory allocated by Intel MKL memory allocator %d bytes. Start "
      "to count new memory peak",
      mkl_peak_mem_usage(MKL_PEAK_MEM_RESET));
  a = (double *)mkl_malloc(n * n * sizeof(double), 64);
  a = (double *)mkl_realloc(a, 2 * n * n * sizeof(double));
  mkl_free(a);
  printf(
      "\nPeak memory allocated by Intel MKL memory allocator after reset of "
      "peak memory counter %d bytes\n",
